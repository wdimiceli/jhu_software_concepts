...
# Global query limit, roughly based on the size of the dataset + some wiggle room. 
QUERY_LIMIT = 32_000
...
def setup_db() -> None:
    ...
    statement = sql.SQL("SELECT 1 FROM pg_database WHERE datname=%s LIMIT 1;")
    cur.execute(statement, [PG_DB])
    ...
    if not cur.fetchone():
        ...
        statement = sql.SQL(
            ...
            ).format(sql.Identifier(PG_DB), sql.Identifier(PG_USER))
